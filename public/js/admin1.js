...
"use strict";
	...
	$(document).on("change", "#providers", function() {
		$("#modalSchedLunch textarea").html(getMenuItemsForTextarea($("#providers").select2("val")));
	});
	 ...
	 function showScheduleModal(dateYMD,numOrders,provID,addlText,extendedText) {
		...
		if (numOrders > 0)
			res += '<select id="providers" name="provider" disabled>';
		else
			res += '<select id="providers" name="provider">';
		...
		$("#providers").select2({minimumResultsForSearch: -1});
	...
	$(document).on("click touchstart", "#scheduletable .nle a, #scheduletable .nleorange a", function(e) {
	 	...
	 	var jqParent = jqThis.parent();
       	...
       	$.ajax({
			...
			$("#gradesteachers").select2({minimumResultsForSearch: -1});
			$('#modalSchedNLE').modal();	
	 	});
	});
	...
	$(document).on("change", "#paymentaccountnames", function() {
		var account_id=$("#paymentaccountnames").select2("val");
		/*if (account_id == 0) { //currently this case cannot happen
			var res = '';
			for (var i=1;i < 6;i++) {
	...
	$(document).on("click", "#tblPayments .btn-danger", function() {
		...
		var t1 = $(this).parent().parent().children(":first-child").html();
		var t2 = $(this).parent().parent().children(".desc").html();
		var t3 = $(this).parent().parent().children(".amt").html();
		var t4 = $(this).parent().parent().children(".date").html();
		var msg = '<p>Account:<br />'+$("#paymentaccountnames option:selected").text()+'</p>';
		msg += '<p>Type: '+t1+'</p>';
		if (t2 != '')
	...
	$(document).on("click", "#btnPayDelete", function() {
		...
		$.ajax({
			type: "GET",
			url: 'deletepayment',
			data: 'account_id='+$("#paymentaccountnames").select2("val")+'&id='+editDeletePaymentID
		})
		.done(function(ajaxResults) {
			updatePaymentsHTML(ajaxResults);
		});
	...
	$(document).on("click", "#tblPayments .btn-success", function() {
		editDeletePaymentID = 0;
		$('#btnPayAddEdit').removeClass().addClass('btn btn-success');
		...
		$('#modalAddEditPayment #accountname').val($("#paymentaccountnames option:selected").text());
		$("#payDesc").val('');
		var momToday = moment();
	...
	$(document).on("click", "#tblPayments .btn-primary", function() {
		...
		$('#btnPayAddEdit').removeClass().addClass('btn btn-primary');
		...
		$('#modalAddEditPayment #accountname').val($("#paymentaccountnames option:selected").text());
		$("#payMeth").select2("val",$(this).parent().data("paymeth"));
		$("#payDesc").val($(this).parent().parent().children(".desc").html());
		$("#payDate").val($(this).parent().parent().children(".date").html());
		$("#payAmt").val($(this).parent().parent().children(".amt").html());
	...
	$(document).on("click", "#btnPayAddEdit", function() {
		...
		var payMeth= $("#payMeth").select2("val");
		...
		$.ajax({
			...
			data: 'id='+editDeletePaymentID+'&payMeth='+payMeth+'&payAmt='+payAmt*100+'&payDesc='+payDesc+'&dateYMD='+
				momDate.format("YYYY-MM-DD")+'&account_id='+$("#paymentaccountnames").select2("val")
		})
		.done(function(ajaxResults) {
			updatePaymentsHTML(ajaxResults);
		});
	...
	$(document).on("change", "#admreportselect", function() {
		var reportid = parseInt($("#admreportselect").select2("val"));
		$("#admreportdate").select2("val",0);
		$("#admreportaccount").select2("val",0);
		$("#admreportdata").html('');
	...
	$(document).on("change", "#admreportdate, #admreportaccount", function() {
		var reportid = parseInt($("#admreportselect").select2("val"));
		var dateYMD = $("#admreportdate").select2("val");
		var account_id = $("#admreportaccount").select2("val");
		$("#admreportdata").html('');
		var needData = false;
		switch (reportid) {
	...
	$(document).on("change", "#lunchordersselectdate", function() {
		var dateYMD = $("#lunchordersselectdate").select2("val");
		if (dateYMD == 0) {
	...
	$(document).on("click touchstart", "#btnDoAction.btn-success", function() {
		$.ajax({
			...
			data: 'dateYMD='+$("#lunchordersselectdate").select2("val")
		})
		.done(function(ajaxResults) {
			$("#lunchordersresults").html(ajaxResults);
		});
	...
	function editAccount(el) {
		...
		$("#uactive").select2("val",el.data("uactive"));
		$("#nnorders").select2("val",el.data("nnorders"));
		$("#atype").select2("val",el.data("atype")); 
		if (el.data("acount") == 0) {
	...
	function addUser(arec) {
	 ...
	 function editUser(el) {
	 	...
	 	$("#ato").select2("val",el.data("ato"));
	 	$("#tid").select2("val",el.data("tid"));
	 	$("#utype").select2("val",utype);
	 	if (utype != 1)
	...
	$(document).on("change", "#utype", function() {
		var utype = parseInt($("#utype").select2("val"));
		if (utype != 1)
			$("#grpuserteachers").addClass('hide');
		else {
			$("#grpuserteachers").removeClass('hide');
	...
	function doAdminInit() {
		...
		if ($("#paymentaccountnames").length > 0)
			$("#paymentaccountnames").select2({minimumResultsForSearch: -1});
			
		if ($("#payMeth").length > 0)
			$("#payMeth").select2({minimumResultsForSearch: -1});
			
		if ($("#admreportselect").length > 0) {
			$("#admreportselect").select2({minimumResultsForSearch: -1});
			$("#admreportdate").select2({minimumResultsForSearch: -1});
			$("#admreportaccount").select2({minimumResultsForSearch: -1});
		}
		if ($("#lunchordersselectdate").length > 0) {
			$("#lunchordersselectdate").select2({minimumResultsForSearch: -1});
		}
        if ($("#select-send-mail-to").length > 0) {
            $("#select-send-mail-to").select2({minimumResultsForSearch: -1});
        }
		if ($("#modalAccount").length > 0) {
			$("#uactive").select2({minimumResultsForSearch: -1});
			$("#atype").select2({minimumResultsForSearch: -1});
			$("#nnorders").select2({minimumResultsForSearch: -1});
			
			$("#utype").select2({minimumResultsForSearch: -1});
			$("#tid").select2({minimumResultsForSearch: -1});
			$("#ato").select2({minimumResultsForSearch: -1});				
		}
	}	
	doAdminInit();
});