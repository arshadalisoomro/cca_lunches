...
class LoginModel
{
    ...
    private function pword_verify($plaintext,$pwd,$use_new_pwd) {
        if ($use_new_pwd == 1) {
            return password_verify($plaintext, $pwd);
        } else {
            $parts	= explode(':', $pwd);
            $crypt	= $parts[0];
            $salt	= @$parts[1];
            $testcrypt = ($salt) ? md5($plaintext . $salt) : md5($plaintext);
    ...
    public function login()
    {
        ...
        $sth = $this->db->prepare("SELECT id as account_id,
                                          user_name,
                                          account_name,
                                          user_email,
                                          user_password_hash,
        ...
        if ($this->pword_verify($_POST['user_password'], $result->user_password_hash, $result->use_new_password)) {
        //if (password_verify($_POST['user_password'], $result->user_password_hash)) {
            ...
            Session::set('account_id', $result->account_id);
            Session::set('user_name', $result->user_name);
            Session::set('account_name', $result->account_name);
            Session::set('user_email', $result->user_email);
            Session::set('user_account_type', $result->user_account_type);
            Session::set('user_provider_type', 'DEFAULT');
            // put native avatar path into session
            Session::set('user_avatar_file', $this->getUserAvatarFilePath());
			Session::set('local_tzo', $_POST['local_tzo']);
            // put Gravatar URL into session
            $this->setGravatarImageUrl($result->user_email, AVATAR_SIZE);

            // reset the failed login counter for that user (if necessary)
            if ($result->user_last_failed_login > 0) {
                $sql = "UPDATE los_accounts SET user_failed_logins = 0, user_last_failed_login = NULL
                        WHERE id = :account_id AND user_failed_logins != 0";
                $sth = $this->db->prepare($sql);
                $sth->execute(array(':account_id' => $result->account_id));
            }
            ...
            $sql = "UPDATE los_accounts SET user_last_login_timestamp = :user_last_login_timestamp WHERE id = :account_id";
            $sth = $this->db->prepare($sql);
            $sth->execute(array(':account_id' => $result->account_id, ':user_last_login_timestamp' => $user_last_login_timestamp));
            ...
            if (isset($_POST['user_rememberme'])) {
                ...
                $sql = "UPDATE los_accounts SET user_rememberme_token = :user_rememberme_token WHERE id = :account_id";
                $sth = $this->db->prepare($sql);
                $sth->execute(array(':user_rememberme_token' => $random_token_string, ':account_id' => $result->account_id));

                // generate cookie string that consists of user id, random string and combined hash of both
                $cookie_string_first_part = $result->account_id . ':' . $random_token_string;
    ...
    public function loginWithCookie()
    {
        ...
        list ($account_id, $token, $hash) = explode(':', $cookie);
        if ($hash !== hash('sha256', $account_id . ':' . $token)) {
            $_SESSION["feedback_negative"][] = FEEDBACK_COOKIE_INVALID;
            return false;
        }
        ...
        $query = $this->db->prepare("SELECT id as account_id, user_name, account_name, user_email, user_password_hash, user_active,
                                     ...
                                     WHERE id = :account_id
                                       ...
                                       AND user_provider_type = :provider_type");
        $query->execute(array(':account_id' => $account_id, ':user_rememberme_token' => $token, ':provider_type' => 'DEFAULT'));
        ...
        if ($count == 1) {
            ...
            $sql = "UPDATE los_accounts SET user_last_login_timestamp = :user_last_login_timestamp WHERE id = :account_id";
            $sth = $this->db->prepare($sql);
            $sth->execute(array(':account_id' => $account_id, ':user_last_login_timestamp' => $user_last_login_timestamp));
    ...
    public function loginWithFacebook()
    {
        ...
        if ($user) {
            try {
                ...
                // write timestamp of this login into database (we only write "real" logins via login form into the
                // database, not the session-login on every page request
                $sql = "UPDATE los_accounts SET user_last_login_timestamp = :user_last_login_timestamp WHERE id = :id";
                $sth = $this->db->prepare($sql);
    ...
    public function editUserName()
    {
        ...
        $query = $this->db->prepare("SELECT id as account_id FROM los_accounts WHERE user_name = :user_name");
        ...
        $query = $this->db->prepare("UPDATE los_accounts SET user_name = :user_name WHERE id = :account_id");
        $query->execute(array(':user_name' => $user_name, ':account_id' => $_SESSION['account_id']));
        $count =  $query->rowCount();
        if ($count == 1) {
    ...
    public function editUserEmail()
    {
        ...
        $query = $this->db->prepare("SELECT * FROM los_accounts WHERE user_email = :user_email");
        ...
        $query = $this->db->prepare("UPDATE los_accounts SET user_email = :user_email WHERE id = :account_id");
        $query->execute(array(':user_email' => $user_email, ':account_id' => $_SESSION['account_id']));
        $count =  $query->rowCount();
        if ($count != 1) {
            $_SESSION["feedback_negative"][] = FEEDBACK_UNKNOWN_ERROR;
            return false;
    ...
    public function registerNewUser()
    {
        ...
        if (!$this->checkCaptcha()) {
            ...
            $query = $this->db->prepare("SELECT * FROM los_accounts WHERE user_name = :user_name");
            $query->execute(array(':user_name' => $user_name));
            $count =  $query->rowCount();
            if ($count == 1) {
                $_SESSION["feedback_negative"][] = FEEDBACK_USERNAME_ALREADY_TAKEN;
            ...
            $query = $this->db->prepare("SELECT id as account_id FROM los_accounts WHERE user_email = :user_email");
            ...
            $sql = "INSERT INTO los_accounts (user_name, user_password_hash, user_email, user_creation_timestamp, user_activation_hash, user_provider_type)
            ...
            $account_id = $result_user_row->account_id;

            // send verification email, if verification email sending failed: instantly delete the user
            if ($this->sendVerificationEmail($account_id, $user_email, $user_activation_hash)) {
                $_SESSION["feedback_positive"][] = FEEDBACK_ACCOUNT_SUCCESSFULLY_CREATED;
                return true;
            } else {
                $query = $this->db->prepare("DELETE FROM los_accounts WHERE id = :last_inserted_id");
                $query->execute(array(':last_inserted_id' => $account_id));
     ...
     */
    private function sendVerificationEmail($account_id, $user_email, $user_activation_hash)
    {
        ...
        $mail->Body = EMAIL_VERIFICATION_CONTENT . EMAIL_VERIFICATION_URL . '/' . urlencode($account_id) . '/' . urlencode($user_activation_hash);
     ...
     * @return bool success status
     */
    public function verifyNewUser($account_id, $user_activation_verification_code)
    {
        $sth = $this->db->prepare("UPDATE los_accounts
                                   SET user_active = 1, user_activation_hash = NULL
                                   WHERE id = :account_id AND user_activation_hash = :user_activation_hash");
        $sth->execute(array(':account_id' => $account_id, ':user_activation_hash' => $user_activation_verification_code));
    ...
    {
        $query = $this->db->prepare("SELECT user_has_avatar FROM los_accounts WHERE id = :account_id");
        $query->execute(array(':account_id' => $_SESSION['account_id']));

        if ($query->fetch()->user_has_avatar) {
            return URL . AVATAR_PATH . $_SESSION['account_id'] . '.jpg';
        } else {
    ...
    public function createAvatar()
    {
        ...
        if ($image_proportions['mime'] == 'image/jpeg' || $image_proportions['mime'] == 'image/png') {
            ...
            $target_file_path = AVATAR_PATH . $_SESSION['account_id'] . ".jpg";
            $this->resizeAvatarImage($_FILES['avatar_file']['tmp_name'], $target_file_path, AVATAR_SIZE, AVATAR_SIZE, AVATAR_JPEG_QUALITY, true);
            $query = $this->db->prepare("UPDATE los_accounts SET user_has_avatar = TRUE WHERE id = :account_id");
            $query->execute(array(':account_id' => $_SESSION['account_id']));
            Session::set('user_avatar_file', $this->getUserAvatarFilePath());
    ...
    public function requestPasswordReset()
    {
        ...
        $query = $this->db->prepare("SELECT user_email FROM los_accounts
    ...
    {
        $query_two = $this->db->prepare("UPDATE los_accounts
    ...
    public function verifyPasswordReset($user_name, $verification_code)
    {
        ...
        $query = $this->db->prepare("SELECT user_password_reset_timestamp
                                       FROM los_accounts
    ...
    public function changeAccountType()
    {
        if (isset($_POST["user_account_upgrade"]) AND !empty($_POST["user_account_upgrade"])) {
            ...
            $query = $this->db->prepare("UPDATE los_accounts SET user_account_type = 2 WHERE id = :account_id");
            $query->execute(array(':account_id' => $_SESSION["account_id"]));
            ...
            $query = $this->db->prepare("UPDATE los_accounts SET user_account_type = 1 WHERE id = :account_id");
            $query->execute(array(':account_id' => $_SESSION["account_id"]));
    ...
    public function registerNewUserWithFacebook($facebook_user_data)
    {
        ...
        $sql = "INSERT INTO los_accounts (user_name, user_email, user_creation_timestamp, user_active, user_provider_type, user_facebook_uid)
        ...
        $count = $query->rowCount();
        if ($count == 1) {
            $query = $this->db->prepare("SELECT id, user_name, user_email, user_account_type, user_provider_type
                                         FROM   los_accounts
                                         WHERE  user_name = :user_name AND user_provider_type = :provider_type");
    ...
    {
        $query = $this->db->prepare("SELECT id FROM los_accounts WHERE user_facebook_uid = :user_facebook_uid");
    ...
    public function facebookUserNameExistsAlreadyInDatabase($facebook_user_data)
    {
        ...
        $query = $this->db->prepare("SELECT id FROM los_accounts WHERE user_name = :clean_user_name_from_facebook");
    ...
    {
        $query = $this->db->prepare("SELECT id FROM los_accounts WHERE user_email = :facebook_email");
